...
中配置了RocketMQ连接地址等信息，需要根据使用情况进行修改，具体参见[9.配置文件说明](#9配置文件说明)。
...
**注意**：不能有空行（demo程序遇到空行会报错）。source connector会持续读取源端文件，每读取到一行数据就会转换为消息体发送到RocketMQ，供sink
connector消费。
...
> {"status":200,"body":{"connector.class":"org.apache.rocketmq.connect.file.FileSourceConnector","filename":"
> /Users/YourUsername/rocketmqconnect/test-source-file.txt","connect.topicname":"fileTopic"}}
...
curl请求返回status:200则表示创建成功，返回样例：
> {"status":200,"body":{"connector.class":"org.apache.rocketmq.connect.file.FileSinkConnector","filename":"
> /Users/YourUsername/rocketmqconnect/test-sink-file.txt","connect.topicnames":"fileTopic"}}
...
**注意**：文件内容可能顺序不一样，这是因为 `rocketmq-connect-sample` 向RocketMQ
...
**注意**：source/sink配置文件说明是以rocketmq-connect-sample为demo，不同source/sink connector配置有差异，请以具体sourc/sink
connector 为准
...
connect-standalone.conf配置文件中， 配置了 [RESTful](https://restfulapi.cn/) 端口，storeRoot 路径，Nameserver
地址等信息，可根据需要进行修改。
