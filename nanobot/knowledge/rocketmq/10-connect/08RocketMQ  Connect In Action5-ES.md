...
- -v /Users/YourUsername/rocketmqconnect/es/es2_data:/usr/share/elasticsearch/data:
  将主机上的一个目录挂载到容器内的/usr/share/elasticsearch/data目录，用于持久化存储Elasticsearch数据。

通过以上命令，您可以运行一个带有自定义配置和数据存储的Elasticsearch容器，并且可以通过主机的9200端口访问其HTTP
...
**注意**：`rocketmq-connect-elasticsearch` 存在一个限制，就是数据中必须要一个可用于 >= 比较运算的字段（字符串 或
数字），该字段会被用于记录同步的位点信息。
